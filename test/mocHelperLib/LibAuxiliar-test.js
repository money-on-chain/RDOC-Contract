...
contract('MoCHelperLib: Auxiliar functions', function([owner]) {
  ...
  describe('Leverage calculation', function() {
    ...
    states.forEach(state => {
      describe(`GIVEN nReserve is ${state.nReserve} and leverage is ${state.leverage}`, function() {
        beforeEach(function() {
          contractReadyState = mocHelper.getContractReadyState(state);
        });
        it(`THEN the amount of ReserveTokens to move from C0 to X2 is ${state.expect.nReserve}`, async function() {
          const { nReserve, leverage, expect } = contractReadyState;
          const transferAmount = await mocHelperLib.bucketTransferAmount(
            toContractBN(nReserve),
            toContractBN(leverage)
          );
          mocHelper.assertBig(transferAmount, expect.nReserve, 'Transfer amount is not correct');
        });
      });
    });
    describe('GIVEN leverage is 1', function() {
      beforeEach(function() {
        contractReadyState = mocHelper.getContractReadyState({ nReserve: 1, leverage: 1 });
      });
      it('THEN the transaction should revert', async function() {
        const { nReserve, leverage } = contractReadyState;
        const tx = mocHelperLib.bucketTransferAmount(
          toContractBN(nReserve),
          toContractBN(leverage)
        );
    ...
    describe('GIVEN leverage is infinite', function() {
      it('THEN the amount of ReserveTokens to move from C0 to X2 is 0', async function() {
        const { nReserve } = mocHelper.getContractReadyState({ nReserve: 1 });
