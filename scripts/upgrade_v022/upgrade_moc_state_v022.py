...
Title: Upgrade v022 Back to normality
Project: ROC
Networks: rdocTestnetAlpha, rdocTestnet, rdocMainnet
...
moc_state_address = Web3.toChecksumAddress(
    connection_manager.options['networks'][network]['addresses']['MoCState'])
...
# Deploy new contract
print("Deploying new contract MoCState version 022...")
path_to_json = os.path.join(path_build, "MoCState.json")
...
# Upgrade Contract
print("Upgrading proxy: {0} to new implementation {1}".format(moc_state_address, new_contract_address))
path_to_json = os.path.join(path_build, "MoCState_v022_Updater.json")
sc = connection_manager.load_bytecode_contract_file_json(path_to_json, link_library=link_library)
tx_hash = connection_manager.fnx_constructor(sc, moc_state_address, upgradeDelegator_address, new_contract_address)
tx_receipt = connection_manager.wait_transaction_receipt(tx_hash)
print(tx_receipt)
